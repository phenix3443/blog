...
context 在 golang 中的主要作用是在不同的 goroutine 之间同步特定数据、取消信号以及请求的截止日期。
...
Cause 返回一个非空错误，解释为什么 c 被取消。首次取消 c 时或其父级的设置了原因。如果该取消是通过调用 CancelCauseFunc(err) 发生的，那么 Cause 返回 err。否则，Cause(c) 返回与 c.Err() 相同的值。如果 c 还未被取消，Cause 返回 nil。
