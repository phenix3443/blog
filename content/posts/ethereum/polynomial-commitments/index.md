...
其中 $y_n$ 对应 32 bytes 的整数。同时，我们可以将上面的数组用点值表示：
...
- 揭示 (Reveal) 阶段：
  ...
  公式一恒等式，所以任意点都成立。在 r 处自然也是成立的，所以如果想要检验 $f(x)$ 在承诺之后是否发生了变化，只需
...
这种方法采用部分揭示方式验证，使得多项式增加了隐私性，自始至终没有完全暴露最初的多项式 $f(x)$。现在已经比较接近 Kate 承诺的方案了。
