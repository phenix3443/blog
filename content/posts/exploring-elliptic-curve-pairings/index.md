...
作为各种构造背后的关键密码原语，包括确定性阈值签名、zk-SNARKs 以及其他更简单形式的零知识证明，椭圆曲线配对是其中的一种。椭圆曲线配对（或“[双线性映射（Bilinear Mapping）]({{< ref "../cryptography#bilinear_mapping" >}})”）是对使用椭圆曲线进行加密和数字签名的 30 年历史的最新补充；配对引入了一种“加密乘法”的形式，大大扩展了基于椭圆曲线的协议可以做的事情。本文的目的将是详细介绍椭圆曲线配对，并概述它们的工作原理的。
...
这篇文章首先简单回顾了椭圆曲线的基本性质，并引出了使用配对的作用：对椭圆曲线上的点做二次约束检查。配对实际上一个双线性的映射函数。接着介绍如何构造配对函数，这个过程中首先介绍了素数域和扩展域的概念，接着介绍了椭圆曲线配对函数的构造。
