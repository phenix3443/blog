...
date: 2023-09-26T14:29:44+08:00
image:
math:
license:
hidden: false
...
我们先整体地看一下在以太坊中区块是如何组织成链的。然后再深入代码细节，去讨论一些具体的问题。

在以太坊中，区块可能会组织成下面这个样子：

![blockchain](images/chain.png)

这个图体现了以太坊区块链的大多数特性。大多数区块组成了一个链条，每个区块指向了自己的`父块（parent）`，直到`创世块（Genesis）`。但也很容易注意到，这个链条从头到尾并不是只有一条，而是有不少“毛刺”似的或长或短的分支链。这些分支链条被称为`侧链 (sidechain)`，而主要的那个链条则是`主链（也称为规范链：canonical chain)`，而这种出现分支链的情况就叫做`分叉 (fork)`。
...
另外关于`修剪 (prune)`的内容没能在这个图上体现出来。以太坊的`state`存储了所有以太坊的账户信息，`state`底层使用`trie`对象存储。由于`trie`的机制和以太坊日益增长的数据，存储全部 state 数据需要非常大的磁盘空间。因此以太坊的区块增加了“修剪”`state`的功能：即那些比较旧的区块的`state`是不存储的，只存储比较新的区块的`state`。我们后面还会详细分析这块功能。
...
+ `core/`（仅包含目录下的 go 文件）：包含了几乎所有重要功能，是以太坊区块链的核心代码：
...
现在看下一个完整的区块 [Block](https://github.com/phenix3443/go-ethereum/blob/0f3580d1a0b79b2cbae40bec8c31333d18c444df/core/types/block.go#L193) 包含了哪些数据：

![block](images/header.webp)

### BlockChain

[BlockChain](https://github.com/phenix3443/go-ethereum/blob/0f3580d1a0b79b2cbae40bec8c31333d18c444df/core/blockchain.go#L202)
...
geth 在 [启动时](https://github.com/phenix3443/go-ethereum/blob/0f3580d1a0b79b2cbae40bec8c31333d18c444df/eth/backend.go#L208) 通过 chaindata 和 genesis 信息创建 blockchain.

在以太坊的区块链代码中，插入区块的方法是`BlockChain.InsertChain`，但主要的代码是由`BlockChain.insertChain`完成的。

![insert bloc](images/chain.drawio.svg)
...
+ [以太坊源码解析：blockchain](http://yangzhe.me/2019/03/24/ethereum-blockchain/)
