...
geth中关于 RPC 的代码位于`rpc/`目录下，rpc 包实现了多种传输方式（http/ws/ipc）上的双向 JSON-RPC 2.0 协议。

- `client.go` 封装了访问 RPC 服务的客户端，常用的功能 `Dial,Call,Subscribe` 都在其中。
- `handler.go` 处理服务端收到 RPC 请求。
- `http.go` HTTP 传输实现。
- `ipc.go` IPC 传输实现。
- `websocket.go` websocket 传输实现。
- `server.go` 将 api 对象转换为 Server 对象对外提供访问。
...
- Hello 用于普通 RPC 调用。
- Clock 用于订阅（subscribe），有关订阅的更多信息，请参阅 [RPC订阅](https://github.com/ethereum/go-ethereum/wiki/RPC-PUB-SUB)
