...
注意：deposited transaction 特指 L2 交易，deposit 可以指各个阶段的交易（比如deposit L1时）。
...
+ User-deposited：`keccak256(bytes32(uint256(0)), keccak256(l1BlockHash, bytes32(uint256(l1LogIndex))))`。其中`l1BlockHash`、`l1LogIndex`都来自 L1 上deposit 的日志事件。`l1LogIndex` 是deposit 事件日志在区块日志事件组合列表中的索引。
+ L1 attributes deposited：`keccak256(bytes32(uint256(1)), keccak256(l1BlockHash), bytes32(uint256(seqNumber)))`。其中 `l1BlockHash` 指存放信息属性的 L1 区块哈希。 `seqNumber = l2BlockNum - l2EpochStartBlockNum`，其中`l2BlockNum`为 L2 中包含 deposit tx的 L2 区块号，`l2EpochStartBlockNum` 为 epoch 中第一个 L2 区块的区块号。
...
+ `CALLER` 和 `ORIGIN` 设置为 `from`
  + deposit feed contract 日志中的`from`没有变化（尽管地址可能已被 deposit feed contract 别名）。
+ `context.calldata` 设置为 `data`
+ `context.gas` 设置为 `gasLimit`
+ `context.value` 设置为 `sendValue`

L2 不购买 gas，也不提供退款。用于 deposit 的 gas 从 L2 上的 gas 池中减去。 Gas 使用量与其他交易类型（包括固有 gas）完全匹配。如果 deposit 耗尽 gas 或发生其他故障，mint 将成功并且帐户的随机数将增加，但不会发生其他状态转换。

如果 deposit 中的 `isSystemTransaction` 设置为 `true`，则 deposit 使用的 gas 是不计量的。一定不能从 gas pool 中减去，收据的`usedGas`字段必须设置为0。

应用程序开发人员注意事项：因为 `CALLER` 和 `ORIGIN` 设置为 `from`，使用 `tx.origin == msg.sender` 检查的语义将无法确定deposit 交易期间调用者是否是为 EOA。相反，检查只能用于识别 L2  deposit 交易中的第一次调用。但是，此检查仍然满足开发人员使用此检查来确保 `CALLER` 无法在调用前后执行代码的常见情况。

#### Nonce Handling

尽管缺少签名验证，我们仍然会在执行 deposit 交易时增加`from`帐户的随机数。在仅 deposit rollup 的情况下，这对于交易排序或重放预防不是必需的，但是它与在[合约创建](https://github.com/ethereum/execution-specs/blob/617903a8f8d7b50cf71bf1aa733c37897c8d75c1/src/ethereum/frontier/utils/address.py#L40)期间使用 nonce 保持一致。它还可以简化与下游工具（例如钱包和区块浏览器）的集成。

## L1 Attributes Deposited Transaction {#l1-attributes-deposited-transaction}

An L1 attributes deposited transaction 是发送到 L1 attributes predeployed contract 的 deposit transaction.
...
+ `data` is an ABI encoded call to the L1 attributes predeployed contract's setL1BlockValues() function with correct values associated with the corresponding L1 block (cf. reference implementation).
...
存款人账户是一个没有已知私钥的 EOA。它的地址是 `0xdeaddeaddeaddeaddeaddeaddeaddeaddeaddead0001`。它的值在执行 L1 attributes deposited transaction 期间由 `CALLER` 和 `ORIGIN` 操作码返回。
...
+ L1块属性：
  ...
  + basefee (uint256)
  + hash (bytes32)
+ sequenceNumber (uint64)： 等于相对于 epoch 开始的 L2 块编号，即 L2 块到 L1 属性最后更改的 L2 块高度的距离，并在新 epoch 开始时重置为 0。
...
该合约实施了一种授权方案，因此它只接受来自[depositor account]({{#l1-attributes-depositor-account}})的状态改变调用。

合约有如下solidity接口，可以按照合约ABI规范进行交互。
...
+ `from` 与发出的值没有变化（尽管它可能已转换为存款馈送合同中的别名）。
...
+ `data` is unchanged from the emitted value. Depending on the value of isCreation it is handled as either calldata or contract initialization code.
...
Deposit contract 负责维护维护[guaranteed gas market](https://github.com/ethereum-optimism/optimism/blob/develop/specs/guaranteed-gas-market.md)，对L2上要使用的gas收取押金，保证单个L1区块的担保gas 总量不超过L2区块 gas limit。

Deposit Contract 处理两种特殊情况：

+ 合约创建存款，通过将 `isCreation` 标志设置为`true`来指示。如果`to`地址不为零，合同将恢复。
+ 来自合约账户的调用，在这种情况下，from 值被转换为其 L2 [alias]({{#address-aliasing}})。
