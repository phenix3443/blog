...
Execution-payloads/blocks 使用交易的最小编码。在交易池和本地交易日志中，则使用网络编码。

对于以前的交易类型，网络编码没有区别，即 `TransactionNetworkPayload == TransactionPayload`。

`TransactionNetworkPayload` 用额外的数据包装 `TransactionPayload`：这个包装数据应该在签名验证之前或之后直接进行验证。

当一个 blob 交易通过网络时（见下面的[network](https://eips.ethereum.org/EIPS/eip-4844#networking)部分），`TransactionNetworkPayload` 版本的交易也包括 `blob` 和 `kzgs`（承诺列表）。执行层在签名验证后根据内部 `TransactionPayload` 验证包装的有效性，因为：
...
- KZG 承诺与 blob 内容相匹配。(注意：这可以用额外的数据进行优化，使用从承诺和 blob 数据得出的两点随机计算的证明)

通过下面的市里验证签名和计算 tx.origin ：
...
通过一个新的 256 位无符号整数字段 `excess_data_gas` 来扩展当前的 header 编码。这是自该 EIP 被激活以来，链上消耗的过量数据气体的运行总量。如果数据气体总量低于目标值， `excess_data_gas` 的上限为 0。

因此，这使得 header RLP 编码是：
...
现在， Blobs 在共识层中被引用，但没有在 beacon block body 中完全编码的。没有将其全部内容嵌入 body 中，而是将 Blobs 的内容作为一个 “sidecar” 单独传播。
...
- 信标链：处理更新的信标块，并确保 Blobs 是可用的。
- P2P 网络：八卦和同步更新的信标块类型和新的 Blobs 侧写。
- 诚实的验证者：产生带有 Blobs 的信标块，发布 Blobs 的侧线。

### Opcode to get versioned hashes

我们添加了一个操作码 `DATAHASH`(其字节值为 `HASH_OPCODE_BYTE`)，该操作码从堆栈顶部读取 `index` 为大端 uint256，如果 `index<len(tx.message.blob_versioned_hashes)`，则将其替换为堆栈中的 `tx.message.blob_versioned_hashes[index]`，否则为零的 bytes32 值。该操作码的气体成本为 `HASH_OPCODE_GAS`。

预编译花费 `POINT_EVALUATION_PRECOMPILE_GAS` 并执行以下逻辑：
...
在 POINT_EVALUATION_PRECOMPILE_ADDRESS 处增加一个预编译，验证一个 KZG 证明，该证明声称一个 blob（由一个承诺代表）在一个给定的点评估为一个给定的值。

预编译的代价是 POINT_EVALUATION_PRECOMPILE_GAS 并执行以下逻辑：
...
我们将 data gas 作为一种新的气体类型来介绍。它独立于普通气体，并遵循自己的目标规则，类似于 EIP-1559。我们使用 `excess_data_gas` 标头字段来存储计算数据气体价格所需的持久性数据。目前，只有 Blobs 是以数据气体来定价的。
...
节点不得自动向其同伴广播 blob 事务。相反，这些交易只能通过 NewPooledTransactionHashes 消息来宣布，然后可以通过 GetPooledTransactions 手动请求。
...
该 EIP 引入了 blob 交易，其格式与最终分片规范中预期存在的格式相同。这为卷积提供了一个临时但重要的扩展救济，允许它们最初扩展到每槽 0.25MB，单独的收费市场允许收费非常低，而这个系统的使用是有限的。

滚动式扩展的核心目标是提供临时的扩展救济，而不给滚动式扩展带来额外的开发负担，以利用这种救济。今天，卷积使用 calldata。在未来，卷积将别无选择，只能使用分片数据（也称为 "blob"），因为分片数据将更加便宜。因此，在这一过程中，rollups 无法避免对其处理数据的方式进行大规模的升级，至少有一次。但我们可以做的是，确保滚动只需要升级一次。这立即意味着正好有两种可能的权宜之计：（i）减少现有 calldata 的气体成本，以及（ii）提前提出将用于分片数据的格式，但尚未实际分片。以前的 EIP 都是第（一）类的解决方案；这个 EIP 是第（二）类的解决方案。

设计这个 EIP 的主要权衡是，现在实现更多与以后必须实现更多：我们是在实现完全分片的过程中实现 25%的工作，还是 50%，还是 75%？

这个 EIP 中已经完成的工作包括：

- 一个新的交易类型，其格式与 "完全分片 "中需要存在的完全相同
- 全分片所需的所有执行层逻辑
- 全分片所需的所有执行/共识交叉验证逻辑
- BeaconBlock 验证和数据可用性采样 blobs 之间的层分离
- 完全分片所需的大部分 BeaconBlock 逻辑
- 为 Blobs 提供一个自我调整的独立气体价格

要实现完全分片，还需要做的工作包括：

- 共识层中 blob_kzgs 的低度扩展，允许 2D 采样
- 数据可用性采样的实际实现
...
- 它增加了一个 SSZ 交易类型，并为所有新的交易类型应该是 SSZ 铺平了道路。
- 它定义了 TransactionNetworkPayload，以分离交易类型的网络和区块编码。
- 它的（更清洁的）天然气价格更新规则可以适用于主要的基本费用

### How rollups would function

与其将滚动块数据放在交易的 calldata 中，滚动将期望滚动块提交者将数据放在 blob 中。这保证了可用性（这也是 rollups 需要的），但比 calldata 要便宜得多。卷积需要数据一次可用，足够长的时间以确保诚实的行为者能够构建卷积状态，但不是永远。

乐观的 rollups 只需要在提交欺诈证明时实际提供基础数据。欺诈证明可以在较小的步骤中验证过渡，每次最多通过 calldata 加载 blob 的几个值。对于每个值，它将提供一个 KZG 证明，并使用点评估预编译来验证该值与之前提交的版本化哈希值，然后像今天这样对该数据进行欺诈证明验证。

ZK 滚动将为他们的交易或状态 delta 数据提供两个承诺：blob 中的 kzg 和使用 ZK 滚动内部使用的任何证明系统的一些承诺。他们将使用承诺的等价证明协议，使用点评估预编译，以证明 kzg（该协议确保指向可用数据）和 ZK rollup 自己的承诺是指相同的数据。

### Versioned hashes & precompile return data

我们使用版本化的哈希值（而不是 kzgs）作为执行层中对 blob 的引用，以确保对未来变化的向前兼容。例如，如果我们由于量子安全的原因需要切换到 Merkle 树+STARKs，那么我们将添加一个新的版本，允许点评估预编译与新格式一起工作。卷积将不必对它们的工作方式进行任何 EVM 级别的改变；定序器将只需在适当的时候切换到使用新的事务类型。

然而，点评估发生在一个有限字段内，而且只有在字段模数已知的情况下，它才会被很好地定义。智能合约可以包含一个将承诺版本映射到模数的表格，但这将不允许智能合约考虑到未来升级到一个尚不知道的模数。通过允许访问 EVM 内部的模数，可以构建智能合约，使其能够使用未来的承诺和证明，而不需要升级。

为了不增加另一个预编译，我们直接从点评估预编译中返回模子和多项式程度。然后它可以被调用者使用。这也是 "免费 "的，因为调用者可以直接忽略返回值的这一部分而不产生额外的费用--在可预见的未来仍然可以升级的系统可能会暂时使用这种途径。

### Data gasprice update rule

数据气体价格更新规则是为了接近公式 data_gasprice = MIN_DATA_GASPRICE \* e\*\*( excess_data_gas / DATA_GASPRICE_UPDATE_FRACTION)，其中 excess_data_gas 是相对于 "目标 "数量（每块 TARGET_DATA_GAS_PER_BLOCK），链所消耗的数据气体总 "额外 "数量。就像 EIP-1559 一样，这是一个自我修正的公式：随着过剩量的增加，data_gasprice 会呈指数级增长，减少使用量，最终迫使过剩量回落。

每个区块的行为大致如下。如果 N 块消耗了 X 个数据气体，那么在 N+1 块中，多余的数据气体增加了 X-TARGET_DATA_GAS_PER_BLOCK，因此 N+1 块的数据气体价格增加了 e\*\*((X-TARGET_DATA_GAS_PER_BLOCK) / DATA_GASPRICE_UPDATE_FRACTION) 的系数。因此，它与现有的 EIP-1559 有类似的效果，但更 "稳定"，因为它对相同的总使用量有相同的反应，不管它是如何分配的。

参数 DATA_GASPRICE_UPDATE_FRACTION 控制 blob 气价的最大变化率。它的目标是每块的最大变化率为 e(TARGET_DATA_GAS_PER_BLOCK / DATA_GASPRICE_UPDATE_FRACTION)≈1.125。
...
该 EIP 引入了一种交易类型，它有不同的 mempool 版本（BlobTransactionNetworkWrapper）和执行支付版本（SignedBlobTransaction），两者之间只能单向转换。blobs 在 BlobTransactionNetworkWrapper 中，而不是在 SignedBlobTransaction 中；相反，它们进入了 BeaconBlockBody。这意味着，现在有一部分交易将无法从 web3 API 中访问。

### Mempool issues

Blob 事务在 mempool 层有很大的数据量，这带来了 mempool DoS 的风险，虽然不是前所未有的风险，因为这也适用于有大量 calldata 的事务。

通过只广播 blob 交易的公告，接收节点将可以控制接收哪些交易和多少交易，使他们可以将吞吐量控制在一个可接受的水平。EIP-5793 将通过扩展 NewPooledTransactionHashes 公告消息以包括交易类型和大小，给节点提供进一步的细粒度控制。
...
这个 EIP 使每个信标块的存储需求最大增加了 0.5MB。这比现在一个块的理论最大尺寸（30M 气体/每 calldata 字节 16 气体=1.875M 字节）大 4 倍，所以它不会大大增加最坏情况下的带宽。合并后，区块时间预计是静态的，而不是不可预测的泊松分布，为大型区块的传播提供了一个保证期。
