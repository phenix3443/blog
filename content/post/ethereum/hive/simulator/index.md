...
simulator 是针对 hive 提供的基于 HTTP simulation  API [^2] 编写的程序。simulator 可以用任何编程语言编写，只要它们是使用 docker 打包的。
...
接下来，simulator 可以在套件上启动测试用例。测试用例被命名并且还有一个由 API 服务器分配的 ID。多个测试用例可能随时在单个套件中运行。请注意，测试套件没有总体通过/失败状态，只有测试用例有。一个套件必须至少启动一个测试用例，否则无法报告任何结果。
...
这是 hive 在启动 simulator 时设置的所有环境变量的列表。
...
虽然 simulator 可以用任何语言编写（毕竟它们只是 docker 容器），但 hive 提供了一个 Go 库，它以类似于标准库“testing”包的方式包装 simulation  API[^2]。请务必查看 [hivesim package](https://pkg.go.dev/github.com/ethereum/hive/hivesim) 的 Go API 参考以获取有关在 Go 中编写 simulator 的更多信息。

simulator 作为独立的 Go 模块包含在 hive 仓库中。 要创建一个，首先在 `./simulators` 中创建一个新的子目录并初始化一个 Go 模块：
