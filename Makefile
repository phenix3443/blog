...
bootstrap: ## 启动 Bootstrap 主题服务器 (端口 1313)
	hugo server --config "config/bootstrap/hugo.yaml" --port 1313

fixit: ## 启动 FixIt 主题服务器 (端口 1314)
	hugo server --config "config/fixit/hugo.yaml" --port 1314

next: ## 启动 Next 主题服务器 (端口 1315)
	hugo server --config "config/next/hugo.yaml" --port 1315

build-bootstrap: ## 构建 Bootstrap 主题
	hugo --config "config/bootstrap/hugo.yaml" --gc --minify

build-fixit: ## 构建 FixIt 主题
	hugo --config "config/fixit/hugo.yaml" --gc --minify

build-next: ## 构建 Next 主题
	hugo --config "config/next/hugo.yaml" --gc --minify
